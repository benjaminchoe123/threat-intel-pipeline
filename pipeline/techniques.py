...
def render(counts, confirmed_only=False):
    ...
    lines = [
        ...
        "#",
        "# Generated by `python -m pipeline.techniques` in threat-intel-pipeline, which",
        "# ingests CISA KEV, abuse.ch ThreatFox/URLhaus/MalwareBazaar and",
        "# malware-traffic-analysis.net. Public ATT&CK identifiers only -- no indicators,",
        "# no customer data.",
        "#",
    ...
    for tid, n in rows:
        # Named here because this repo carries the ATT&CK catalog and ruleproof
        # deliberately does not. An id the catalog does not know stays unnamed
        # rather than guessed -- the same rule the enrichment itself follows.
        name = attack.name_for(tid)
        lines.append(f"{tid}  # x{n}" + (f"  {name}" if name else ""))
