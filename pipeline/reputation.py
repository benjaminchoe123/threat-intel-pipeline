...
from . import abuseipdb, epss, greynoise, vt

log = logging.getLogger(__name__)


def reputation_for_item(item, vt_fn, abuse_fn, epss_fn=None, greynoise_fn=None):
    ...
    if greynoise_fn is not None:
        providers.append(("GreyNoise", greynoise_fn))
...
def default_reputation(item, cache=None):
    ...
    return reputation_for_item(
        ...
        greynoise_fn=lambda i: greynoise.block_for_item(
            i, api_key=config.GREYNOISE_API_KEY, cache=cache
        ),
